      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ContactCard
            ...
            email={item.email}
            image={item.image}
            onPress={() => { navigation.navigate("RequestMoneyAmount") }}
          />
        )}
      />
    </View>
...
const RequestMoney: React.FC = () => {
  ...
  const renderScene = ({ route }: { route: { key: string } }) => {
    switch (route.key) {
      ...
      case 'third':
        return <BankContact contacts={filteredFavouriteContacts} />;
      case 'fourth':
        return <EWalletContact contacts={filteredFavouriteContacts} />;
      default:
        return null;
    }
  };
    ...
    <SafeAreaView style={[styles.area, { backgroundColor: colors.background }]}>
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={[styles.searchBarContainer, { backgroundColor: dark ? COLORS.dark2 : COLORS.secondaryWhite }]}>
          ...
          <TouchableOpacity>
            <Image
              source={icons.microphone as ImageSourcePropType}
              resizeMode='contain'
